...
openssl req -x509 -new -nodes -key ${CERTS_DIR}/${ROOTCA}.key -sha256 -days 1024 -out ${CERTS_DIR}/${ROOTCA}.pem -subj "/C=IT/ST=ANCONA/L=ANCONA/O=EUPROJECT/OU=IT Dev/CN=${HOSTNAME}"

openssl genrsa -out ${CERTS_DIR}/ssl.key 2048

openssl req -new -key ${CERTS_DIR}/ssl.key -out ${CERTS_DIR}/ssl.csr -subj "/C=IT/ST=ANCONA/L=ANCONA/O=EUPROJECT/OU=IT Dev/CN=${HOSTNAME}"
